...
@SpringBootTest
@ActiveProfiles("test")
class GrupoConsistenciaGatewayTest {

    @Autowired
    private GrupoConsistenciaGateway gateway;

    @Test
    void sucessoTotal_grupo200() {
        var request = new GrupoConsistenciaRequest(
        ...
        assertThat(response.consistencias()).isNotEmpty();
        assertThat(response.consistencias()).allMatch(c -> "SUCESSO".equals(c.estadoTecnico()));
    ...
    void reproducaoNegocio_clienteBaixoScore() {
        ...
        var scoreResult = response.consistencias().stream()
                .filter(c -> "calcular_score_interno".equals(c.nomeConsistencia()))
                .findFirst();
    ...
    void sucesso_grupo300() {
        ...
        assertThat(response.estadoGrupo()).isEqualTo("FINALIZADO_SUCESSO");
        assertThat(response.resultadoNegocioGrupo()).isEqualTo("APROVADO");
        assertThat(response.idGrupoConsistencia()).isEqualTo(300);
    }
    ...
    void sucesso_grupo100() {
        ...
        assertThat(response.idGrupoConsistencia()).isEqualTo(100);
    }
